...
class CloudFormationConvergerWithRole
  ...
  def converge_and_update_template(create_template_string:, create_parameters:, update_template_string:, update_parameters:)
    ...
    admin_role_arn = admin_role.create(iam_client: @iam_client)
  ...
  def converge_template(template_string:, parameters:, role_name: role_name)
    role_name = new_role_name(role_name)
    admin_role = AdminRole.new
  ...
  def self.iam_role_arn(account_id, role_id)
    "arn:aws:iam::#{account_id}:role/av-core-domain-cfn-leaprog-#{role_id}"
  end

  private

  def new_role_name(role_name='')
    puts DateTime.parse(Time.now.to_s)
    "#{role_name}-#{Time.now.to_i}"
  end
end
