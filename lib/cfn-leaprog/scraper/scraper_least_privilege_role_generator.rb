...
class ScraperLeastPrivilegeRoleGenerator
  ...
  def initialize
    @role_name = 'av-core-domain-cfn-leaprog'
    @failed_stack_ids = []
  ...
  def update_stack_role(create_template_string:,
    ...
    stack = create_stack_with_wait(
      ...
      stack_name: "av-core-domain-cfn-leaprog#{Time.now.to_i}",
      template_string: create_template_string,
      parameters: create_parameters,
  ...
  def create_stack_role(template_string:,
    ...
    logger.info('av-core-domain-cfn-leaprog is reset')
    ...
    while true
      unless sufficient_actions.empty?
        logger.info("Converging av-core-domain-cfn-leaprog with actions: #{sufficient_actions}")
