...
namespace :ct do
  ...
  task :infra do
    region = 'us-east-1'
    trail_stack_name = 'av-core-domain-cfn-leaprog-trail'
    lambda_stack_name = 'av-core-domain-cfn-leaprog-lambda'
  ...
  puts JSON.pretty_generate( YAML.load(PolicyRenderer.new.render(role_and_policy, request_parameters)))
  end

  desc 'Generate the least privilege IAM policy to create a CloudFormation template by parsing CloudTrail'
  task :create_stack, [:template_path, :parameters_path, :role_name] do |_, args|
    if args[:parameters_path]
      parameters_string = IO.read(args[:parameters_path])
      parameters = CloudFormation.convert_parameters(parameters_string)
    else
